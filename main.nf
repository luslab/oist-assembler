...
include { bowtie2_build } from "./modules/luslab-nf-modules/tools/bowtie2/main.nf"
include { bowtie2_align } from "./modules/luslab-nf-modules/tools/bowtie2/main.nf"
include { pilon } from "./modules/luslab-nf-modules/tools/pilon/main.nf"
...
//    justMinimapPaf = minimap2_paf.out.paf.map { row -> row[1] }
    ...
    // 0. Assess BUSCO before polishing
    busco_genome0(busco_genome0_opts, fastq_metadata.out)
    // 1. Make bowtie2 index of genome, which is the new data1 value
    bowtie2_build(params.modules['bowtie2_build'], just_fasta)
    // 2. Align illumina reads to genome with bowtie2
    bowtie2_align(params.modules['bowtie2_align'], ch_illumina_for_bt2, bowtie2_build.out.bowtieIndex.collect())
    // 3. Run Pilon on metadata+fasta and metadata+bam tuples
    pilon(params.modules['pilon'], fastq_metadata.out, bowtie2_align.out.bam)
    // 4. Assess BUSCO after polishing
    busco_genome1(busco_genome1_opts, pilon.out.fasta)
